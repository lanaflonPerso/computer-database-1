...
public final class ServletUtil {
	...
	public static CorrectField checkComputerDto(ComputerDto computerDto) {
		...
		if (computerDto.getIntroduced() != null && !Validator.isDateValid(computerDto.getIntroduced())) {
			incorrectField.setIntroducedDateTrue(false);
		}
		if (computerDto.getDiscontinued() != null && !Validator.isDateValid(computerDto.getDiscontinued())) {
			incorrectField.setDiscontinuedDateTrue(false);
		}
		return incorrectField;
	}
	
	public static ComputerPage getAddComputerPage(
		...
		page.setCorrectField(ServletUtil
				.checkComputerDto(page.getComputerDto()));
		return page;
	}

	public static ComputerPage getEditComputerPageGet(
			HttpServletRequest request) {
		ComputerPage page = new ComputerPage();
		List<Company> companies = CompanyService.INSTANCE
				.list(new SortCriteria());
	...
	public static ComputerPage getEditComputerPagePost(
		...
		Computer c = ComputerService.INSTANCE.getById(Long.valueOf(page.getComputerDto().getId()));
