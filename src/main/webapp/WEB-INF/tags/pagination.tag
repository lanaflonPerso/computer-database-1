...
<div class="container text-center">
	<ul class="pagination">
		<c:if test="${page != 1}">
			<li><a href="dashboard?page=${page-1}&size=${size}&search=${search}" aria-label="Previous">
		...
		<c:if test="${page-2 > 0}">
			<li><a id="switchPageMalus2"
				href="dashboard?page=${page-2}&size=${size}&search=${search}">${page-2}</a></li>
		</c:if>
		<c:if test="${page-1 > 0}">
			<li><a id="switchPageMalus1"
				href="dashboard?page=${page-1}&size=${size}&search=${search}">${page-1}</a></li>
		</c:if>
		...
		<c:if test="${page != pageMax}">
			<li><a href="dashboard?page=${page+1}&size=${size}&search=${search}" aria-label="Next"> <span
